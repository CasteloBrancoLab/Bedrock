...
public sealed class ViolationManager
{
    ...
    /// <summary>
    /// Todas as violações coletadas (snapshot thread-safe).
    /// </summary>
    public IReadOnlyList<Violation> Violations
    {
        get { lock (_lock) { return [.. _violations]; } }
    }

    /// <summary>
    /// Todos os resultados de análise por regra/projeto (snapshot thread-safe).
    /// </summary>
    public IReadOnlyList<RuleAnalysisResult> RuleResults
    {
        get { lock (_lock) { return [.. _ruleResults]; } }
    }
    ...
    {
        get { lock (_lock) { return _violations.Exists(v => v.Severity == Severity.Error); } }
    }
