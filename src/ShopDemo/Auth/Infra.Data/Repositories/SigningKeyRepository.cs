...
public sealed class SigningKeyRepository
    : RepositoryBase<SigningKey>,
    ISigningKeyRepository
{
    private readonly ISigningKeyPostgreSqlRepository _postgreSqlRepository;

    public SigningKeyRepository(
        ILogger<SigningKeyRepository> logger,
        ISigningKeyPostgreSqlRepository postgreSqlRepository
    ) : base(logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.GetActiveAsync(
                executionContext,
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting active signing key.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.GetByKidAsync(
                executionContext,
                kid,
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting signing key by kid.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.GetAllValidAsync(
                executionContext,
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting all valid signing keys.");
            return [];
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while updating signing key.");
            return false;
        }
    }
    ...
    protected override async IAsyncEnumerable<SigningKey> GetAllInternalAsync(
    ...
    protected override Task<SigningKey?> GetByIdInternalAsync(
        ExecutionContext executionContext,
        Id id,
        CancellationToken cancellationToken)
    {
    ...
    // Stryker disable all : Metodo stub sem implementacao - mutantes equivalentes (yield break sem corpo)
    [ExcludeFromCodeCoverage(Justification = "Metodo stub sem implementacao - async iterator gera branch inalcancavel no state machine")]
    protected override async IAsyncEnumerable<SigningKey> GetModifiedSinceInternalAsync(
        ExecutionContext executionContext,
