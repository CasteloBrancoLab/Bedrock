...
public sealed class ServiceClientRepository
    : RepositoryBase<ServiceClient>,
    IServiceClientRepository
{
    private readonly IServiceClientPostgreSqlRepository _postgreSqlRepository;

    public ServiceClientRepository(
        ILogger<ServiceClientRepository> logger,
        IServiceClientPostgreSqlRepository postgreSqlRepository
    ) : base(logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.GetByClientIdAsync(
                executionContext,
                clientId,
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting service client by client ID.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.GetByCreatorUserIdAsync(
                executionContext,
                createdByUserId,
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting service clients by creator user ID.");
            return [];
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while updating service client.");
            return false;
        }
    }
    ...
    protected override async IAsyncEnumerable<ServiceClient> GetAllInternalAsync(
    ...
    protected override Task<ServiceClient?> GetByIdInternalAsync(
        ExecutionContext executionContext,
        Id id,
        CancellationToken cancellationToken)
    {
    ...
    // Stryker disable all : Metodo stub sem implementacao - mutantes equivalentes (yield break sem corpo)
    [ExcludeFromCodeCoverage(Justification = "Metodo stub sem implementacao - async iterator gera branch inalcancavel no state machine")]
    protected override async IAsyncEnumerable<ServiceClient> GetModifiedSinceInternalAsync(
        ExecutionContext executionContext,
