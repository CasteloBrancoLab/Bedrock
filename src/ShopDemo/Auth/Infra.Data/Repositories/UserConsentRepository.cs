...
public sealed class UserConsentRepository
    : RepositoryBase<UserConsent>,
    IUserConsentRepository
{
    private readonly IUserConsentPostgreSqlRepository _postgreSqlRepository;

    public UserConsentRepository(
        ILogger<UserConsentRepository> logger,
        IUserConsentPostgreSqlRepository postgreSqlRepository
    ) : base(logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting user consents by user ID.");
            return [];
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.GetActiveByUserIdAndConsentTermIdAsync(
                executionContext,
                userId,
                consentTermId,
                cancellationToken);
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting active user consent by user ID and consent term ID.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while updating user consent.");
            return false;
        }
    }
    ...
    protected override async IAsyncEnumerable<UserConsent> GetAllInternalAsync(
    ...
    protected override Task<UserConsent?> GetByIdInternalAsync(
        ExecutionContext executionContext,
        Id id,
        CancellationToken cancellationToken)
    {
    ...
    // Stryker disable all : Metodo stub sem implementacao - mutantes equivalentes (yield break sem corpo)
    [ExcludeFromCodeCoverage(Justification = "Metodo stub sem implementacao - async iterator gera branch inalcancavel no state machine")]
    protected override async IAsyncEnumerable<UserConsent> GetModifiedSinceInternalAsync(
        ExecutionContext executionContext,
