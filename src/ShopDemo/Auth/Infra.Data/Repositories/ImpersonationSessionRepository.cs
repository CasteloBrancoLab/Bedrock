...
public sealed class ImpersonationSessionRepository
    : RepositoryBase<ImpersonationSession>,
    IImpersonationSessionRepository
{
    private readonly IImpersonationSessionPostgreSqlRepository _postgreSqlRepository;

    public ImpersonationSessionRepository(
        ILogger<ImpersonationSessionRepository> logger,
        IImpersonationSessionPostgreSqlRepository postgreSqlRepository
    ) : base(logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.GetActiveByOperatorUserIdAsync(
                executionContext,
                operatorUserId,
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting active impersonation session by operator user ID.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.GetActiveByTargetUserIdAsync(
                executionContext,
                targetUserId,
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting active impersonation session by target user ID.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while updating impersonation session.");
            return false;
        }
    }
    ...
    protected override async IAsyncEnumerable<ImpersonationSession> GetAllInternalAsync(
    ...
    protected override Task<ImpersonationSession?> GetByIdInternalAsync(
        ExecutionContext executionContext,
        Id id,
        CancellationToken cancellationToken)
    {
    ...
    // Stryker disable all : Metodo stub sem implementacao - mutantes equivalentes (yield break sem corpo)
    [ExcludeFromCodeCoverage(Justification = "Metodo stub sem implementacao - async iterator gera branch inalcancavel no state machine")]
    protected override async IAsyncEnumerable<ImpersonationSession> GetModifiedSinceInternalAsync(
        ExecutionContext executionContext,
