...
public sealed class ApiKeyRepository
    : RepositoryBase<ApiKey>,
    IApiKeyRepository
{
    private readonly IApiKeyPostgreSqlRepository _postgreSqlRepository;

    public ApiKeyRepository(
        ILogger<ApiKeyRepository> logger,
        IApiKeyPostgreSqlRepository postgreSqlRepository
    ) : base(logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.GetByKeyHashAsync(
                executionContext,
                keyHash,
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting API key by key hash.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting API keys by service client ID.");
            return [];
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while updating API key.");
            return false;
        }
    }
    ...
    protected override async IAsyncEnumerable<ApiKey> GetAllInternalAsync(
    ...
    protected override Task<ApiKey?> GetByIdInternalAsync(
        ExecutionContext executionContext,
        Id id,
        CancellationToken cancellationToken)
    {
    ...
    // Stryker disable all : Metodo stub sem implementacao - mutantes equivalentes (yield break sem corpo)
    [ExcludeFromCodeCoverage(Justification = "Metodo stub sem implementacao - async iterator gera branch inalcancavel no state machine")]
    protected override async IAsyncEnumerable<ApiKey> GetModifiedSinceInternalAsync(
        ExecutionContext executionContext,
