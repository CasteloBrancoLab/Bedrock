...
public sealed class ClaimRepository
    : RepositoryBase<Claim>,
    IClaimRepository
{
    private readonly IClaimPostgreSqlRepository _postgreSqlRepository;

    public ClaimRepository(
        ILogger<ClaimRepository> logger,
        IClaimPostgreSqlRepository postgreSqlRepository
    ) : base(logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting claim by name.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting all claims.");
            return [];
        }
    }
    ...
    protected override async IAsyncEnumerable<Claim> GetAllInternalAsync(
    ...
    protected override Task<Claim?> GetByIdInternalAsync(
        ExecutionContext executionContext,
        Id id,
        CancellationToken cancellationToken)
    {
    ...
    // Stryker disable all : Metodo stub sem implementacao - mutantes equivalentes (yield break sem corpo)
    [ExcludeFromCodeCoverage(Justification = "Metodo stub sem implementacao - async iterator gera branch inalcancavel no state machine")]
    protected override async IAsyncEnumerable<Claim> GetModifiedSinceInternalAsync(
        ExecutionContext executionContext,
