...
public sealed class MfaSetupRepository
    : RepositoryBase<MfaSetup>,
    IMfaSetupRepository
{
    private readonly IMfaSetupPostgreSqlRepository _postgreSqlRepository;

    public MfaSetupRepository(
        ILogger<MfaSetupRepository> logger,
        IMfaSetupPostgreSqlRepository postgreSqlRepository
    ) : base(logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while getting MFA setup by user ID.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while updating MFA setup.");
            return false;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await _postgreSqlRepository.DeleteByUserIdAsync(
                executionContext,
                userId,
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            Logger.LogExceptionForDistributedTracing(
                executionContext,
                ex,
                "An error occurred while deleting MFA setup by user ID.");
            return false;
        }
    }
    ...
    protected override async IAsyncEnumerable<MfaSetup> GetAllInternalAsync(
    ...
    protected override Task<MfaSetup?> GetByIdInternalAsync(
        ExecutionContext executionContext,
        Id id,
        CancellationToken cancellationToken)
    {
    ...
    // Stryker disable all : Metodo stub sem implementacao - mutantes equivalentes (yield break sem corpo)
    [ExcludeFromCodeCoverage(Justification = "Metodo stub sem implementacao - async iterator gera branch inalcancavel no state machine")]
    protected override async IAsyncEnumerable<MfaSetup> GetModifiedSinceInternalAsync(
        ExecutionContext executionContext,
