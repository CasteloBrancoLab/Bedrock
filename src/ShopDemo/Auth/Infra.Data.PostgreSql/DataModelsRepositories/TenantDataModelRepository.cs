...
public sealed class TenantDataModelRepository
    : DataModelRepositoryBase<TenantDataModel>,
      ITenantDataModelRepository
{
    private readonly IAuthPostgreSqlUnitOfWork _unitOfWork;
    private readonly IDataModelMapper<TenantDataModel> _mapper;

    // Stryker disable once Block : Construtor delega para base class e armazena campos privados - testado indiretamente
    public TenantDataModelRepository(
        ILogger<TenantDataModelRepository> logger,
        IAuthPostgreSqlUnitOfWork unitOfWork,
        IDataModelMapper<TenantDataModel> mapper)
    ...
    public async Task<TenantDataModel?> GetByDomainAsync(
        ExecutionContext executionContext,
        string domain,
        CancellationToken cancellationToken)
    {
        WhereClause whereClause =
            _mapper.Where(static (TenantDataModel x) => x.Domain)
            & _mapper.Where(static (TenantDataModel x) => x.TenantCode);
        ...
        _mapper.AddParameterForCommand(command, static (TenantDataModel x) => x.Domain, domain);
        _mapper.AddParameterForCommand(command, static (TenantDataModel x) => x.TenantCode, executionContext.TenantInfo.Code);
    ...
    // Stryker disable all : Requer conexao PostgreSQL real - coberto por testes de integracao
    [ExcludeFromCodeCoverage(Justification = "Requer conexao PostgreSQL real - coberto por testes de integracao")]
    public async Task<IReadOnlyList<TenantDataModel>> GetAllAsync(
        ExecutionContext executionContext,
        CancellationToken cancellationToken)
    {
        WhereClause whereClause =
        ...
        var results = new List<TenantDataModel>();
