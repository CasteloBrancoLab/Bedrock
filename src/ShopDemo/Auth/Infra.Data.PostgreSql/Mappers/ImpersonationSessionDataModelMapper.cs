...
public sealed class ImpersonationSessionDataModelMapper
    : DataModelMapperBase<ImpersonationSessionDataModel>
{
    protected override void ConfigureInternal(MapperOptions<ImpersonationSessionDataModel> mapperOptions)
    {
        mapperOptions
            .MapTable(schema: "public", name: "auth_impersonation_sessions")
            .MapColumn(static x => x.OperatorUserId)
            .MapColumn(static x => x.TargetUserId)
            .MapColumn(static x => x.ExpiresAt)
            .MapColumn(static x => x.Status)
            .MapColumn(static x => x.EndedAt);
    }
    ...
    public override void MapBinaryImporter(NpgsqlBinaryImporter importer, ImpersonationSessionDataModel model)
    {
        ...
        // ImpersonationSession-specific columns
        importer.Write(model.OperatorUserId, NpgsqlDbType.Uuid);
        importer.Write(model.TargetUserId, NpgsqlDbType.Uuid);
        importer.Write(model.ExpiresAt, NpgsqlDbType.TimestampTz);
        importer.Write(model.Status, NpgsqlDbType.Smallint);
        importer.Write(model.EndedAt, NpgsqlDbType.TimestampTz);
    }
    // Stryker restore all
}
