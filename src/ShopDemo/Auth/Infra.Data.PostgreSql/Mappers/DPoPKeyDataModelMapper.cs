...
public sealed class DPoPKeyDataModelMapper
    : DataModelMapperBase<DPoPKeyDataModel>
{
    protected override void ConfigureInternal(MapperOptions<DPoPKeyDataModel> mapperOptions)
    {
        mapperOptions
            .MapTable(schema: "public", name: "auth_dpop_keys")
            .MapColumn(static x => x.UserId)
            .MapColumn(static x => x.JwkThumbprint)
            .MapColumn(static x => x.PublicKeyJwk)
    ...
    public override void MapBinaryImporter(NpgsqlBinaryImporter importer, DPoPKeyDataModel model)
    {
        ...
        // DPoPKey-specific columns
        importer.Write(model.UserId, NpgsqlDbType.Uuid);
        importer.Write(model.JwkThumbprint, NpgsqlDbType.Varchar);
        importer.Write(model.PublicKeyJwk, NpgsqlDbType.Varchar);
