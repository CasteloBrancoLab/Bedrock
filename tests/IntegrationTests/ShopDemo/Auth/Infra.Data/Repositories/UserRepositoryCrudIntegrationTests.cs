...
[Collection("AuthData")]
[Feature("UserRepository CRUD", "Facade Infra.Data round-trip Domain Entity <-> PostgreSQL")]
public class UserRepositoryCrudIntegrationTests : IntegrationTestBase
{
    private readonly AuthDataFixture _fixture;

    public UserRepositoryCrudIntegrationTests(
    ...
    public async Task GetByIdAsync_Should_ReturnUserEntity_WhenExists()
    {
        ...
        LogArrange("Inserindo usuario via raw SQL e buscando via facade");
        ...
        // Act
        LogAct("Chamando GetByIdAsync na facade");
        ...
        // Assert
        LogAssert("Verificando que a entidade User foi construida corretamente via facade");
    ...
    public async Task ExistsAsync_Should_ReturnTrue_WhenUserExists()
    {
        // Arrange
        UseEnvironment(_fixture.Environments["auth-data"]);
        LogArrange("Inserindo usuario e verificando existencia via facade");
        var tenantCode = Guid.NewGuid();
        var dataModel = _fixture.CreateTestUserDataModel(tenantCode: tenantCode);
        await _fixture.InsertUserDirectlyAsync(dataModel);
    ...
    public async Task ExistsAsync_Should_ReturnFalse_WhenUserNotExists()
    {
        // Arrange
        UseEnvironment(_fixture.Environments["auth-data"]);
        LogArrange("Verificando existencia de usuario inexistente");
        var tenantCode = Guid.NewGuid();
        var executionContext = _fixture.CreateExecutionContext(tenantCode);
        await using var unitOfWork = _fixture.CreateAppUserUnitOfWork();
        var repository = _fixture.CreateUserRepository(unitOfWork);
        ...
        // Act
        LogAct("Chamando ExistsAsync para ID inexistente");
        await unitOfWork.OpenConnectionAsync(executionContext, CancellationToken.None);
        var result = await repository.ExistsAsync(
            executionContext,
            Id.CreateFromExistingInfo(Guid.NewGuid()),
    ...
    public async Task RegisterNewAsync_Should_PersistUserEntity()
    {
        ...
        var email = EmailAddress.CreateNew("facade_test@example.com");
        ...
        // Act
        LogAct("Chamando RegisterNewAsync via facade");
        await unitOfWork.OpenConnectionAsync(executionContext, CancellationToken.None);
        ...
        persisted.Email.ShouldBe("facade_test@example.com");
