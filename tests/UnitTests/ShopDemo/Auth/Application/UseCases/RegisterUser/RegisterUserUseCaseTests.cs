...
namespace ShopDemo.UnitTests.Auth.Application.UseCases.RegisterUser;

public class RegisterUserUseCaseTests : TestBase
{
    private readonly Mock<ILogger<RegisterUserUseCase>> _loggerMock;
    private readonly Mock<IUnitOfWork> _unitOfWorkMock;
    private readonly Mock<IAuthenticationService> _authServiceMock;
    private readonly Mock<IAuthOutboxWriter> _outboxWriterMock;
    private readonly RegisterUserUseCase _sut;

    public RegisterUserUseCaseTests(ITestOutputHelper outputHelper) : base(outputHelper)
    {
        ...
        _unitOfWorkMock
            .Setup(static x => x.ExecuteAsync(
                ...
                It.IsAny<Func<ExecutionContext, RegisterUserInput, CancellationToken, Task<bool>>>(),
                It.IsAny<CancellationToken>()))
            .Returns<ExecutionContext, RegisterUserInput,
                Func<ExecutionContext, RegisterUserInput, CancellationToken, Task<bool>>,
                CancellationToken>(
    ...
    public void Constructor_WithNullLogger_ShouldThrow()
    {
        ...
        Should.Throw<ArgumentNullException>(() =>
            new RegisterUserUseCase(null!, _unitOfWorkMock.Object, _authServiceMock.Object,
    ...
    public void Constructor_WithNullUnitOfWork_ShouldThrow()
    {
        ...
        LogAssert("Verifying ArgumentNullException is thrown");
        Should.Throw<ArgumentNullException>(() =>
            new RegisterUserUseCase(_loggerMock.Object, null!, _authServiceMock.Object,
                _outboxWriterMock.Object, TimeProvider.System));
    }
    ...
    public void Constructor_WithNullAuthService_ShouldThrow()
    {
        ...
        LogAssert("Verifying ArgumentNullException is thrown");
        Should.Throw<ArgumentNullException>(() =>
            new RegisterUserUseCase(_loggerMock.Object, _unitOfWorkMock.Object, null!,
                _outboxWriterMock.Object, TimeProvider.System));
    }
    ...
    public void Constructor_WithNullOutboxWriter_ShouldThrow()
    {
        ...
        LogAssert("Verifying ArgumentNullException is thrown");
        Should.Throw<ArgumentNullException>(() =>
            new RegisterUserUseCase(_loggerMock.Object, _unitOfWorkMock.Object,
    ...
    [Fact]
    public void ShouldImplementIRegisterUserUseCase()
    {
        LogAssert("Verifying interface implementation");
        _sut.ShouldBeAssignableTo<IRegisterUserUseCase>();
    }
    ...
    [Fact]
    public async Task ExecuteAsync_WhenRegistrationSucceeds_ShouldReturnOutput()
    {
        // Arrange
        LogArrange("Setting up successful registration");
        var executionContext = CreateTestExecutionContext();
        var input = new RegisterUserInput("test@example.com", "SecurePassword123!");
        var user = CreateTestUser(executionContext);

        _authServiceMock
            .Setup(x => x.RegisterUserAsync(executionContext, input.Email, input.Password,
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(user);
    ...
    [Fact]
    public async Task ExecuteAsync_WhenRegistrationSucceeds_ShouldEnqueueEvent()
    {
        // Arrange
        LogArrange("Setting up successful registration");
        var executionContext = CreateTestExecutionContext();
    ...
    [Fact]
    public async Task ExecuteAsync_WhenRegistrationFails_ShouldReturnNull()
    {
        // Arrange
        LogArrange("Setting up failed registration (auth service returns null)");
        var executionContext = CreateTestExecutionContext();
        var input = new RegisterUserInput("test@example.com", "SecurePassword123!");
    ...
    [Fact]
    public async Task ExecuteAsync_WhenRegistrationFails_ShouldAddErrorMessage()
    {
        // Arrange
        LogArrange("Setting up failed registration with no pre-existing errors");
        var executionContext = CreateTestExecutionContext();
        var input = new RegisterUserInput("test@example.com", "SecurePassword123!");
    ...
    [Fact]
    public async Task ExecuteAsync_WhenRegistrationFails_ShouldNotEnqueueEvent()
    {
        // Arrange
