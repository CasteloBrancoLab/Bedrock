...
public class PasswordResetTokenDataModelFactoryTests : TestBase
{
    private static readonly Faker Faker = new();

    public PasswordResetTokenDataModelFactoryTests(ITestOutputHelper outputHelper) : base(outputHelper)
    {
    }
    ...
    {
        // Arrange
        LogArrange("Creating PasswordResetToken entity with known UserId");
        var expectedUserId = Guid.NewGuid();
        var entity = CreateTestEntity(expectedUserId, "token-hash", DateTimeOffset.UtcNow.AddHours(1), false, null);

        // Act
        LogAct("Creating PasswordResetTokenDataModel from PasswordResetToken entity");
        var dataModel = PasswordResetTokenDataModelFactory.Create(entity);
    ...
    {
        // Arrange
        LogArrange("Creating PasswordResetToken entity with known TokenHash");
    ...
    {
        // Arrange
        LogArrange("Creating PasswordResetToken entity with known ExpiresAt");
    ...
    {
        // Arrange
        LogArrange($"Creating PasswordResetToken entity with IsUsed={expectedIsUsed}");
        var usedAt = expectedIsUsed ? DateTimeOffset.UtcNow.AddMinutes(-10) : (DateTimeOffset?)null;
        var entity = CreateTestEntity(Guid.NewGuid(), "token-hash", DateTimeOffset.UtcNow.AddHours(1), expectedIsUsed, usedAt);
    ...
    {
        // Arrange
        LogArrange("Creating PasswordResetToken entity with known UsedAt");
        var expectedUsedAt = DateTimeOffset.UtcNow.AddMinutes(-5);
    ...
    {
        // Arrange
        LogArrange("Creating PasswordResetToken entity with null UsedAt");
    ...
    {
        // Arrange
        LogArrange("Creating PasswordResetToken entity with specific EntityInfo values");
