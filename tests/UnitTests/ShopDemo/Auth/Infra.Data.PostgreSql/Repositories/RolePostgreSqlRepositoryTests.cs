...
public class RolePostgreSqlRepositoryTests : TestBase
{
    private static readonly Faker Faker = new();

    private readonly Mock<IRoleDataModelRepository> _dataModelRepositoryMock;
    private readonly RolePostgreSqlRepository _repository;

    public RolePostgreSqlRepositoryTests(ITestOutputHelper outputHelper) : base(outputHelper)
    {
        _dataModelRepositoryMock = new Mock<IRoleDataModelRepository>();
        _repository = new RolePostgreSqlRepository(_dataModelRepositoryMock.Object);
    }
    ...
    public void Constructor_WithNullDataModelRepository_ShouldThrowArgumentNullException()
    {
        ...
        // Act
        LogAct("Attempting to create RolePostgreSqlRepository with null");
        var action = () => new RolePostgreSqlRepository(null!);
    ...
    {
        // Arrange
        LogArrange("Creating mock IRoleDataModelRepository");
        var dataModelRepositoryMock = new Mock<IRoleDataModelRepository>();

        // Act
        LogAct("Creating RolePostgreSqlRepository");
        var repository = new RolePostgreSqlRepository(dataModelRepositoryMock.Object);
    ...
    [Fact]
    public async Task GetByIdAsync_WhenDataModelFound_ShouldReturnRole()
    {
        // Arrange
        LogArrange("Setting up mock to return a RoleDataModel");
        ...
        // Assert
        LogAssert("Verifying Role was created from DataModel");
    ...
    public async Task GetByIdAsync_WhenDataModelNotFound_ShouldReturnNull()
    {
        ...
        _dataModelRepositoryMock
            .Setup(static x => x.GetByIdAsync(
                ...
                It.IsAny<Guid>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync((RoleDataModel?)null);
    ...
    {
        // Arrange
        LogArrange("Creating Role entity and setting up mock");
        var executionContext = CreateTestExecutionContext();
        ...
        _dataModelRepositoryMock
            .Setup(static x => x.InsertAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<RoleDataModel>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);

        // Act
        LogAct("Calling RegisterNewAsync");
        var result = await _repository.RegisterNewAsync(executionContext, role, CancellationToken.None);

        // Assert
        LogAssert("Verifying InsertAsync was called with a RoleDataModel");
        result.ShouldBeTrue();
        _dataModelRepositoryMock.Verify(
            static x => x.InsertAsync(
                It.IsAny<ExecutionContext>(),
                It.Is<RoleDataModel>(static dm => dm.Name == "Admin"),
                It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task UpdateAsync_WhenExistingDataModelFound_ShouldAdaptAndUpdate()
    {
        ...
        var role = CreateTestRole(entityId: entityId);
        ...
        var result = await _repository.UpdateAsync(executionContext, role, CancellationToken.None);
    ...
    public async Task UpdateAsync_ShouldPassExistingDataModelVersionToDataModelRepository()
    {
        ...
        var role = CreateTestRoleWithVersion(entityId, dbVersion + 100);
        ...
        // Act
        LogAct("Calling UpdateAsync");
        await _repository.UpdateAsync(executionContext, role, CancellationToken.None);
    ...
    public async Task UpdateAsync_ShouldAdaptDataModelFieldsBeforeUpdate()
    {
        ...
        var role = CreateTestRole(entityId: entityId, name: updatedName, description: updatedDescription);
        ...
        RoleDataModel? capturedDataModel = null;
        _dataModelRepositoryMock
            .Setup(static x => x.UpdateAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<RoleDataModel>(),
                It.IsAny<long>(),
                It.IsAny<CancellationToken>()))
            .Callback<ExecutionContext, RoleDataModel, long, CancellationToken>(
        ...
        capturedDataModel.Name.ShouldBe(updatedName);
        capturedDataModel.Description.ShouldBe(updatedDescription);
    }
    ...
    public async Task DeleteAsync_WhenExistingDataModelFound_ShouldDeleteWithCorrectVersion()
    {
        ...
        var result = await _repository.DeleteAsync(executionContext, role, CancellationToken.None);
    ...
    public async Task EnumerateAllAsync_ShouldDelegateToDataModelRepository()
    {
        ...
        EnumerateAllItemHandler<Role> handler = (_, _, _, _) => Task.FromResult(true);

        _dataModelRepositoryMock
            .Setup(static x => x.EnumerateAllAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<PaginationInfo>(),
                It.IsAny<DataModelItemHandler<RoleDataModel>>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
    ...
    public async Task EnumerateModifiedSinceAsync_ShouldDelegateToDataModelRepository()
    {
        ...
        EnumerateModifiedSinceItemHandler<Role> handler = (_, _, _, _, _) => Task.FromResult(true);
    ...
    {
        // Arrange
        LogArrange("Setting up mock to return RoleDataModel by name");
        var executionContext = CreateTestExecutionContext();
        string roleName = "Administrator";
        var dataModel = CreateTestDataModel(name: roleName);
        ...
        var result = await _repository.GetByNameAsync(executionContext, roleName, CancellationToken.None);

        // Assert
        LogAssert("Verifying Role was returned with matching name");
        result.ShouldNotBeNull();
    ...
    public async Task GetByNameAsync_WhenNotFound_ShouldReturnNull()
    {
        ...
        // Act
        LogAct("Calling GetByNameAsync");
        var result = await _repository.GetByNameAsync(executionContext, "NonExistentRole", CancellationToken.None);
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task ExistsByNameAsync_ShouldDelegateToDataModelRepository(bool expectedResult)
    {
        // Arrange
        LogArrange($"Setting up mock to return {expectedResult}");
        var executionContext = CreateTestExecutionContext();
        string roleName = "CheckRole";

        _dataModelRepositoryMock
            .Setup(static x => x.ExistsByNameAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<string>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(expectedResult);
        ...
        // Act
        LogAct("Calling ExistsByNameAsync");
        var result = await _repository.ExistsByNameAsync(executionContext, roleName, CancellationToken.None);

        // Assert
        LogAssert($"Verifying result is {expectedResult}");
        result.ShouldBe(expectedResult);
        _dataModelRepositoryMock.Verify(
            static x => x.ExistsByNameAsync(
                It.IsAny<ExecutionContext>(),
                It.Is<string>(static n => n == "CheckRole"),
                It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    private static Role CreateTestRoleWithVersion(Guid entityId, long entityVersion)
    {
        var entityInfo = EntityInfo.CreateFromExistingInfo(
