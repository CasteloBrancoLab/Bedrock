...
public class PasswordHistoryPostgreSqlRepositoryTests : TestBase
{
    private static readonly Faker Faker = new();

    private readonly Mock<IPasswordHistoryDataModelRepository> _dataModelRepositoryMock;
    private readonly PasswordHistoryPostgreSqlRepository _repository;

    public PasswordHistoryPostgreSqlRepositoryTests(ITestOutputHelper outputHelper) : base(outputHelper)
    {
        _dataModelRepositoryMock = new Mock<IPasswordHistoryDataModelRepository>();
        _repository = new PasswordHistoryPostgreSqlRepository(_dataModelRepositoryMock.Object);
    }
    ...
    public void Constructor_WithNullDataModelRepository_ShouldThrowArgumentNullException()
    {
        ...
        // Act
        LogAct("Attempting to create PasswordHistoryPostgreSqlRepository with null");
        var action = () => new PasswordHistoryPostgreSqlRepository(null!);
    ...
    {
        // Arrange
        LogArrange("Creating mock IPasswordHistoryDataModelRepository");
        var dataModelRepositoryMock = new Mock<IPasswordHistoryDataModelRepository>();

        // Act
        LogAct("Creating PasswordHistoryPostgreSqlRepository");
        var repository = new PasswordHistoryPostgreSqlRepository(dataModelRepositoryMock.Object);
    ...
    [Fact]
    public async Task GetByIdAsync_WhenDataModelFound_ShouldReturnPasswordHistory()
    {
        // Arrange
        LogArrange("Setting up mock to return a PasswordHistoryDataModel");
        ...
        // Assert
        LogAssert("Verifying PasswordHistory was created from DataModel");
    ...
    public async Task GetByIdAsync_WhenDataModelNotFound_ShouldReturnNull()
    {
        ...
        _dataModelRepositoryMock
            .Setup(static x => x.GetByIdAsync(
                ...
                It.IsAny<Guid>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync((PasswordHistoryDataModel?)null);
    ...
    {
        // Arrange
        LogArrange("Creating PasswordHistory entity and setting up mock");
        var executionContext = CreateTestExecutionContext();
        var passwordHistory = CreateTestPasswordHistory();

        _dataModelRepositoryMock
            .Setup(static x => x.InsertAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<PasswordHistoryDataModel>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
        ...
        // Assert
        LogAssert("Verifying InsertAsync was called with a PasswordHistoryDataModel");
        result.ShouldBeTrue();
        _dataModelRepositoryMock.Verify(
    ...
    public async Task EnumerateAllAsync_ShouldDelegateToDataModelRepository()
    {
        ...
        EnumerateAllItemHandler<PasswordHistory> handler = (_, _, _, _) => Task.FromResult(true);

        _dataModelRepositoryMock
            .Setup(static x => x.EnumerateAllAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<PaginationInfo>(),
                It.IsAny<DataModelItemHandler<PasswordHistoryDataModel>>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
    ...
    public async Task EnumerateModifiedSinceAsync_ShouldDelegateToDataModelRepository()
    {
        ...
        EnumerateModifiedSinceItemHandler<PasswordHistory> handler = (_, _, _, _, _) => Task.FromResult(true);
    ...
    [Fact]
    public async Task GetLatestByUserIdAsync_WhenHistoriesExist_ShouldReturnList()
    {
        // Arrange
        LogArrange("Setting up mock to return list of PasswordHistoryDataModels by userId");
        var executionContext = CreateTestExecutionContext();
        var userId = Guid.NewGuid();
        var userIdObj = Id.CreateFromExistingInfo(userId);
        int count = 5;
        var dataModels = new List<PasswordHistoryDataModel>
        {
        ...
        _dataModelRepositoryMock
            .Setup(x => x.GetLatestByUserIdAsync(
                It.IsAny<ExecutionContext>(), userId, count, It.IsAny<CancellationToken>()))
            .ReturnsAsync(dataModels);

        // Act
        LogAct("Calling GetLatestByUserIdAsync");
        var result = await _repository.GetLatestByUserIdAsync(
            executionContext, userIdObj, count, CancellationToken.None);
    ...
    [Fact]
    public async Task GetLatestByUserIdAsync_WhenNoHistory_ShouldReturnEmptyList()
    {
        // Arrange
        ...
        _dataModelRepositoryMock
            .Setup(static x => x.GetLatestByUserIdAsync(
                It.IsAny<ExecutionContext>(), It.IsAny<Guid>(), It.IsAny<int>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<PasswordHistoryDataModel>());

        // Act
        LogAct("Calling GetLatestByUserIdAsync");
        var result = await _repository.GetLatestByUserIdAsync(
            executionContext, userId, count, CancellationToken.None);
        ...
        Guid? userId = null)
    {
        return new PasswordHistoryDataModel
        {
            ...
            PasswordHash = Faker.Random.AlphaNumeric(64),
            ChangedAt = DateTimeOffset.UtcNow.AddDays(-30)
        };
    }

    private static PasswordHistory CreateTestPasswordHistory(Guid? entityId = null)
    {
