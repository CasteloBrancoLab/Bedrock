...
public class ServiceClientScopePostgreSqlRepositoryTests : TestBase
{
    private static readonly Faker Faker = new();

    private readonly Mock<IServiceClientScopeDataModelRepository> _dataModelRepositoryMock;
    private readonly ServiceClientScopePostgreSqlRepository _repository;

    public ServiceClientScopePostgreSqlRepositoryTests(ITestOutputHelper outputHelper) : base(outputHelper)
    {
        _dataModelRepositoryMock = new Mock<IServiceClientScopeDataModelRepository>();
        _repository = new ServiceClientScopePostgreSqlRepository(_dataModelRepositoryMock.Object);
    }
    ...
    public void Constructor_WithNullDataModelRepository_ShouldThrowArgumentNullException()
    {
        ...
        // Act
        LogAct("Attempting to create ServiceClientScopePostgreSqlRepository with null");
        var action = () => new ServiceClientScopePostgreSqlRepository(null!);
    ...
    {
        // Arrange
        LogArrange("Creating mock IServiceClientScopeDataModelRepository");
        var dataModelRepositoryMock = new Mock<IServiceClientScopeDataModelRepository>();

        // Act
        LogAct("Creating ServiceClientScopePostgreSqlRepository");
        var repository = new ServiceClientScopePostgreSqlRepository(dataModelRepositoryMock.Object);
    ...
    [Fact]
    public async Task GetByIdAsync_WhenDataModelFound_ShouldReturnServiceClientScope()
    {
        // Arrange
        LogArrange("Setting up mock to return a ServiceClientScopeDataModel");
        ...
        // Assert
        LogAssert("Verifying ServiceClientScope was created from DataModel");
    ...
    public async Task GetByIdAsync_WhenDataModelNotFound_ShouldReturnNull()
    {
        ...
        _dataModelRepositoryMock
            .Setup(static x => x.GetByIdAsync(
                ...
                It.IsAny<Guid>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync((ServiceClientScopeDataModel?)null);
    ...
    {
        // Arrange
        LogArrange("Creating ServiceClientScope entity and setting up mock");
        var executionContext = CreateTestExecutionContext();
        var entity = CreateTestServiceClientScope();

        _dataModelRepositoryMock
            .Setup(static x => x.InsertAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<ServiceClientScopeDataModel>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
        ...
        // Assert
        LogAssert("Verifying InsertAsync was called with a ServiceClientScopeDataModel");
        result.ShouldBeTrue();
        _dataModelRepositoryMock.Verify(
            static x => x.InsertAsync(
                It.IsAny<ExecutionContext>(),
                It.Is<ServiceClientScopeDataModel>(static dm => dm.ServiceClientId != Guid.Empty),
                It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    [Fact]
    public async Task GetByServiceClientIdAsync_WhenScopesExist_ShouldReturnList()
    {
        // Arrange
        LogArrange("Setting up mock to return a list of ServiceClientScopeDataModels");
        var executionContext = CreateTestExecutionContext();
        var serviceClientId = Guid.NewGuid();
        var serviceClientIdObj = Id.CreateFromExistingInfo(serviceClientId);
        var dataModels = new List<ServiceClientScopeDataModel>
        {
            CreateTestDataModel(serviceClientId: serviceClientId),
            CreateTestDataModel(serviceClientId: serviceClientId)
        };

        _dataModelRepositoryMock
            .Setup(x => x.GetByServiceClientIdAsync(
                It.IsAny<ExecutionContext>(),
                serviceClientId,
                It.IsAny<CancellationToken>()))
        ...
        var result = await _repository.GetByServiceClientIdAsync(
            executionContext, serviceClientIdObj, CancellationToken.None);
    ...
    [Fact]
    public async Task GetByServiceClientIdAsync_WhenNoScopesExist_ShouldReturnEmptyList()
    {
        // Arrange
        ...
        _dataModelRepositoryMock
            .Setup(static x => x.GetByServiceClientIdAsync(
                ...
                It.IsAny<Guid>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<ServiceClientScopeDataModel>());

        // Act
        LogAct("Calling GetByServiceClientIdAsync");
        var result = await _repository.GetByServiceClientIdAsync(
            executionContext, serviceClientId, CancellationToken.None);
    ...
    #region DeleteByServiceClientIdAsync Tests

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public async Task DeleteByServiceClientIdAsync_ShouldDelegateToDataModelRepository(bool expectedResult)
    {
        // Arrange
        LogArrange($"Setting up mock to return {expectedResult}");
        var executionContext = CreateTestExecutionContext();
        ...
        _dataModelRepositoryMock
            .Setup(static x => x.DeleteByServiceClientIdAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<Guid>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(expectedResult);
        ...
        // Act
        LogAct("Calling DeleteByServiceClientIdAsync");
        var result = await _repository.DeleteByServiceClientIdAsync(
            executionContext, serviceClientId, CancellationToken.None);
        ...
        result.ShouldBe(expectedResult);
        _dataModelRepositoryMock.Verify(
            static x => x.DeleteByServiceClientIdAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<Guid>(),
                It.IsAny<CancellationToken>()),
            Times.Once);
    ...
    public async Task EnumerateAllAsync_ShouldDelegateToDataModelRepository()
    {
        ...
        EnumerateAllItemHandler<ServiceClientScope> handler = (_, _, _, _) => Task.FromResult(true);

        _dataModelRepositoryMock
            .Setup(static x => x.EnumerateAllAsync(
                It.IsAny<ExecutionContext>(),
                It.IsAny<PaginationInfo>(),
                It.IsAny<DataModelItemHandler<ServiceClientScopeDataModel>>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
    ...
    public async Task EnumerateModifiedSinceAsync_ShouldDelegateToDataModelRepository()
    {
        ...
        EnumerateModifiedSinceItemHandler<ServiceClientScope> handler = (_, _, _, _, _) => Task.FromResult(true);
        ...
        Guid? serviceClientId = null)
    {
        return new ServiceClientScopeDataModel
        {
            ...
            EntityVersion = 1,
            ServiceClientId = serviceClientId ?? Guid.NewGuid(),
            Scope = Faker.Internet.DomainWord()
        };
    }

    private static ServiceClientScope CreateTestServiceClientScope(Guid? entityId = null)
    {
