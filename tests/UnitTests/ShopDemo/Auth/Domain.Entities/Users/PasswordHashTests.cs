...
public class PasswordHashTests : TestBase
{
    public PasswordHashTests(ITestOutputHelper outputHelper) : base(outputHelper)
    {
    }
    ...
    {
        // Arrange
        LogArrange("Creating byte array for password hash");
        byte[] hashBytes = [1, 2, 3, 4, 5];

        // Act
        LogAct("Creating PasswordHash via CreateNew");
        var passwordHash = PasswordHash.CreateNew(hashBytes);

        // Assert
        LogAssert("Verifying value is preserved");
        passwordHash.Value.Span.SequenceEqual(hashBytes).ShouldBeTrue();
    }
    ...
    public void CreateNew_ShouldMakeDefensiveCopy()
    {
        ...
        // Act
        LogAct("Creating PasswordHash and modifying original array");
        var passwordHash = PasswordHash.CreateNew(hashBytes);
        hashBytes[0] = 99;

        // Assert
        LogAssert("Verifying PasswordHash was not affected by mutation");
        passwordHash.Value.Span[0].ShouldBe((byte)1);
    }
    ...
    public void CreateNew_WithEmptyArray_ShouldCreateEmptyHash()
    {
        ...
        // Act
        LogAct("Creating PasswordHash from empty array");
        var passwordHash = PasswordHash.CreateNew(hashBytes);

        // Assert
        LogAssert("Verifying hash is empty");
        passwordHash.IsEmpty.ShouldBeTrue();
        passwordHash.Length.ShouldBe(0);
    }
    ...
    {
        // Arrange & Act
        LogAct("Creating default PasswordHash");
    ...
    public void IsEmpty_WithNonEmptyHash_ShouldBeFalse()
    {
        ...
        bool isEmpty = passwordHash.IsEmpty;
    ...
    {
        // Arrange
        LogArrange("Creating hash with 10 bytes");
        byte[] hashBytes = new byte[10];
        var passwordHash = PasswordHash.CreateNew(hashBytes);
        ...
        // Assert
        LogAssert("Verifying length is 10");
        length.ShouldBe(10);
    }
    ...
    public void Equals_WithSameBytes_ShouldReturnTrue()
    {
        ...
        var hash1 = PasswordHash.CreateNew(bytes);
        var hash2 = PasswordHash.CreateNew(bytes);
    ...
    public void Equals_WithDifferentBytes_ShouldReturnFalse()
    {
        ...
        var hash1 = PasswordHash.CreateNew([1, 2, 3]);
        var hash2 = PasswordHash.CreateNew([4, 5, 6]);
    ...
    public void Equals_WithDifferentLengths_ShouldReturnFalse()
    {
        ...
        var hash1 = PasswordHash.CreateNew([1, 2, 3]);
        var hash2 = PasswordHash.CreateNew([1, 2, 3, 4]);
    ...
    public void Equals_WithObjectOfSameType_ShouldReturnTrue()
    {
        ...
        byte[] bytes = [10, 20, 30];
        var hash1 = PasswordHash.CreateNew(bytes);
        object hash2 = PasswordHash.CreateNew(bytes);
    ...
    public void Equals_WithObjectOfDifferentType_ShouldReturnFalse()
    {
        ...
        var hash = PasswordHash.CreateNew([1, 2, 3]);
    ...
    public void ToString_ShouldReturnRedacted()
    {
        ...
        var passwordHash = PasswordHash.CreateNew([1, 2, 3, 4, 5]);
    ...
    {
        // Arrange & Act
        LogAct("Calling ToString on default PasswordHash");
        string result = default(PasswordHash).ToString();
