using System.Text;
using Bedrock.BuildingBlocks.Serialization.Protobuf;
...
namespace Bedrock.UnitTests.BuildingBlocks.Serialization.Protobuf;

public class ProtobufSerializerBaseTests : TestBase
{
    private readonly TestProtobufSerializer _serializer;

    public ProtobufSerializerBaseTests(ITestOutputHelper outputHelper) : base(outputHelper)
    {
        _serializer = new TestProtobufSerializer();
    }
    ...
    #region GenerateProtoFileContent Tests

    [Fact]
    public void GenerateProtoFileContent_ShouldReturnProtoSchema()
    {
        // Arrange
        LogArrange("Preparing to generate proto schema");

        // Act
        LogAct("Generating proto file content");
        var proto = _serializer.GenerateProtoFileContent("Bedrock.Test");

        // Assert
        LogAssert("Verifying proto content");
        proto.ShouldNotBeNullOrEmpty();
        proto.ShouldContain("package");
        LogInfo("Proto generated: {0}", proto.Substring(0, Math.Min(100, proto.Length)));
    }
    ...
    public void Constructor_ShouldCallConfigureInternal()
    {
        ...
        var serializer = new TestProtobufSerializer();
    ...
    public void Name_ShouldBeAccessible()
    {
        ...
        // Act
        LogAct("Accessing name");
        var name = _serializer.Name;

        // Assert
        LogAssert("Verifying name");
        name.ShouldNotBeNullOrEmpty();
        name.ShouldBe("TestProtobufSerializer");
        LogInfo("Name: {0}", name);
    }
    ...
    private class TestProtobufSerializer : ProtobufSerializerBase
    {
        public bool ConfigureInternalWasCalled { get; private set; }

        public TestProtobufSerializer() : base("TestProtobufSerializer")
        {
        }
        ...
        protected override void ConfigureInternal(Options options)
        {
            ...
            options.WithSupportedTypes(typeof(TestDto));
        }
    }
